...
public class GlassCabinetBlockEntity extends BlockEntity implements MenuProvider {

    private static final Component TITLE =
            Component.translatable("container." + MoreDelight.MODID + ".glass_cabinet_block");

    private final ItemStackHandler inventory = new ItemStackHandler(27) {
        @Override
        protected void onContentsChanged(int slot) {
            super.onContentsChanged(slot);
            GlassCabinetBlockEntity.this.setChanged();
        }
    };
    ...
    public GlassCabinetBlockEntity(BlockPos pos, BlockState state) {
        super(BlockEntityInit.GLASS_CABINET_BLOCK_ENTITY.get(), pos, state);
    }


    // Helper method for updating state based on packet
    @Override
    ...
    public void load(@NotNull CompoundTag nbt) {
        ...
        this.inventory.deserializeNBT(moredelightData.getCompound("Inventory"));
    }
    ...
    protected void saveAdditional(@NotNull CompoundTag nbt) {
        ...
        moredelightData.put("Inventory", this.inventory.serializeNBT());
        nbt.put(MoreDelight.MODID, moredelightData);
    }

    public void updateBlockState(boolean open) {
        GlassCabinetBlock.setOpenState(level, worldPosition, level.getBlockState(worldPosition), open);
    }
