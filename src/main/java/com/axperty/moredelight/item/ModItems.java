...
public class ModItems {
    ...
    public static void registerItems() {
        ...
        ModItemList.COOKED_RICE_WITH_MILK_CREAM_AND_CHICKEN_CUTS = foodEffect("cooked_rice_with_milk_cream_and_chicken_cuts", Items.BOWL, 14, 0.8f, ModEffects.NOURISHMENT.get(), 3600, 0, 16);

        ModItemList.COOKED_RICE_WITH_MILK_CREAM_AND_BEEF = foodEffect("cooked_rice_with_milk_cream_and_beef", Items.BOWL, 14, 0.8f, ModEffects.NOURISHMENT.get(), 3600, 0, 16);

        ModItemList.COOKED_RICE_WITH_MILK_CREAM_AND_PORKCHOP = foodEffect("cooked_rice_with_milk_cream_and_porkchop", Items.BOWL, 14, 0.8f, ModEffects.NOURISHMENT.get(), 3600, 0, 16);

        ModItemList.MASHED_POTATOES = food("mashed_potatoes", Items.BOWL, 12, 0.8f, 16);

        ModItemList.PASTA_WITH_MILK_CREAM_AND_HAM = foodEffect("pasta_with_milk_cream_and_ham", Items.BOWL, 12, 0.8f, ModEffects.NOURISHMENT.get(), 3600, 0, 16);
        ...
        ModItemList.CARROT_SOUP = stew("carrot_soup", Items.BOWL, 12, 0.8f, ModEffects.COMFORT.get(), 3600, 0);
    ...
    private static Item food(String name, Item remainder, int hunger, float saturation, int maxCount) {
        Item item = Registry.register(Registries.ITEM, new Identifier(MoreDelight.MOD_ID, name),
                new Item(new FabricItemSettings().recipeRemainder(remainder).maxCount(maxCount)
                        .food(new FoodComponent.Builder().hunger(hunger).saturationModifier(saturation).build())));
        ItemGroupEvents.modifyEntriesEvent(MoreDelight.GROUP).register(entries -> entries.add(item));
        return item;
    }

    private static Item foodEffect(String name, Item remainder, int hunger, float saturation, StatusEffect effect, int duration, int amplifier, int maxCount) {
        Item item = Registry.register(Registries.ITEM, new Identifier(MoreDelight.MOD_ID, name),
                new ModEffectToolTip(effect, duration, amplifier, new FabricItemSettings()
    ...
    private static Item stew(String name, Item remainder, int hunger, float saturation, StatusEffect effect, int duration, int amplifier) {
        Item item = Registry.register(Registries.ITEM, new Identifier(MoreDelight.MOD_ID, name),
                new ModEffectToolTip(effect, duration, amplifier, new FabricItemSettings()
